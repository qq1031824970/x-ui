...
import "x-anything/dist/theme/Button.css"
...
export const Default: Story & { args: { content: string } } = {
  ...
  render: (args) => ({
    ...
    template: container(
      `<x-button data-testid="story-test-btn" v-bind="args">{{args.content}}</x-button>`
    ),
  }),
...
export const Group: Story & { args: { content1: string; content2: string } } = {
  ...
  render: (args) => ({
    ...
    template: container(`
       <x-button-group :type="args.groupType" :size="args.groupSize" :disabled="args.groupDisabled">
         <x-button v-bind="args">{{args.content1}}</x-button>
         <x-button v-bind="args">{{args.content2}}</x-button>
       </x-button-group>
    `),
  }),
  play: async ({ canvasElement, args, step }) => {
